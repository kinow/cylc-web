...
import { createFamilyProxyNode, getCyclePointId } from '@/components/cylc/table/table-nodes'
import Vue from 'vue'
 ...
 * be reactive and used in the node table component).
 *
...
/**
 * @typedef {Object} CylcTable
 * @property {Map<string, Object>} lookup - lookup map used to access objects without iteration
 * @property {TableNode} root - table root
 */

/**
 * A data structure representing the table used by Cylc.
 *
 ...
 * const rootNode = cylcTableInstance.root;
 ...
 * reference.
 *
 * It means that you can easily access any element in the table, without
 * having to iterate and visit each of its parents.
 *
 * <script language="javascript">
 * const jobID = 'kinow|five|20150129T1200+12|foo|1'
 * const someJob = cycleTableInstance.lookup.get(jobID)
 * </script>
 *
 ...
 *
 * When a node is added, it gets added to the hierarchical table (i.e. it
...
class CylcTable {
  /**
   * Create a table with an initial root node, representing
   * a workflow in Cylc.
   *
  ...
  addFamilyProxy (familyProxy) {
    ...
    if (!existingFamilyProxy) {
      ...
      // when linking with the parent node in the table, not the new GraphQL data
    ...
    // nodes if they don't exist in the table yet, so that we can create the correct hierarchy. Later, we
